...
class TestNGramGenerator(TestCase):
    ...
    def test_init_1gram(self):
        ...
        self.assertEqual(dict(generator.probs), probs, msg = dict(generator.probs))
    ...
    def test_generate_token(self):
        ...
        for i in range(100):
            ...
            # after 'la' always comes 'gata':
            token = generator.generate_token(('la',))
            self.assertEqual(token, 'gata')

            # after 'come' may come 'pescado' or 'salmón'
            token = generator.generate_token(('come',))
            self.assertTrue(token in ['pescado', 'salmón'])

            # after '<s>' may come 'el' or 'la'
            token = generator.generate_token(('<s>',))
